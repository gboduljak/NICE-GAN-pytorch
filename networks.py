...
class ResnetGenerator(nn.Module):
  def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, light=False):
    ...
    UpBlock0 = [nn.ReflectionPad2d(1),
                nn.Conv2d(int(ngf * mult / 2), ngf * mult,
    ...
    for i in range(n_downsampling):
      ...
      UpBlock2 += [nn.ReflectionPad2d(1),
                   ...
                   ILN(int(ngf * mult / 2)),
                   nn.ReLU(True),
                   nn.Conv2d(int(ngf * mult / 2), int(ngf * mult / 2)
                             * 4, kernel_size=1, stride=1, bias=True),
                   nn.PixelShuffle(2),
                   ILN(int(ngf * mult / 2)),
                   nn.ReLU(True)
                   ]
...
class adaILN(nn.Module):
  ...
  def forward(self, input, gamma, beta):
    ...
    if self.using_bn:
      if self.training:
        bn_mean, bn_var = torch.mean(input, dim=[0, 2, 3], keepdim=True), torch.var(
            input, dim=[0, 2, 3], keepdim=True)
...
class Discriminator(nn.Module):
  def __init__(self, input_nc, ndf=64, n_layers=7, nce_layers_indices: List[int] = []):
    ...
    Dis1_0 = []
    # 1+3*2^0 + 3*2^1 + 3*2^2 + 3*2^3=46, 1+3*2^0 + 3*2^1 + 3*2^2 +3*2^3 +3*2^4 = 94
    for i in range(n_layers - 4, n_layers - 2):
      mult = 2 ** (i - 1)
    ...
    self.layers = dict(enumerate(self.model))

  def forward(self, input, nce=False):
    if nce:
